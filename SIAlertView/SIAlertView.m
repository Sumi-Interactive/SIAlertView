...
#define MESSAGE_CAN_SCROLL_COUNT 7
#define GAP 10
...
- (void)validateLayout
{
    ...
    if (self.messageLabel) {
        ...
        if (height <= defaultScrollHeight) {
            ...
            self.messageScroll.frame = CGRectMake(CONTENT_PADDING_LEFT, y, self.containerView.bounds.size.width - CONTENT_PADDING_LEFT * 2, defaultScrollHeight);
            self.messageScroll.contentSize = CGSizeMake(self.containerView.bounds.size.width - CONTENT_PADDING_LEFT * 2, height);
            self.messageLabel.frame = CGRectMake(0, 0, self.containerView.bounds.size.width - CONTENT_PADDING_LEFT * 2, height);
            y += defaultScrollHeight;
        }
    }
...
- (CGFloat)heightForMessageLabel
{
    ...
    if (self.messageLabel) {
        ...
        // NSString class method: boundingRectWithSize:options:attributes:context is
        // available only on ios7.0 sdk.
        CGRect rect = [self.message boundingRectWithSize:CGSizeMake(CONTAINER_WIDTH - CONTENT_PADDING_LEFT * 2, CGFLOAT_MAX)
                                                 options:NSStringDrawingUsesLineFragmentOrigin
                                              attributes:attributes
                                                 context:nil];
...
- (void)updateMessageLabel
{
    if (self.message) {
        if (!self.messageLabel) {
            ...
            self.messageLabel.textAlignment =  (self.messageAlignmentStyle > 1) ? NSTextAlignmentRight : ((self.messageAlignmentStyle > 0) ? NSTextAlignmentLeft : NSTextAlignmentCenter);
            ...
            if (height <= defaultScrollHeight) {
                ...
                self.messageScroll = nil;
            } else {
                if (!self.messageScroll) {
                    self.messageScroll = [[UIScrollView alloc] init];
                    [self.containerView addSubview:self.messageScroll];
                    [self.messageScroll addSubview:self.messageLabel];
                }
            }
            
#if DEBUG_LAYOUT
            self.messageScroll.backgroundColor = [UIColor blueColor];
